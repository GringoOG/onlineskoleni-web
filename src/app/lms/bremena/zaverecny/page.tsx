...
export default async function BremenaOfficialTestPage() {
  ...
  const session = await requireOfficialTestAccess(COURSE_SLUG, '/lms/bremena/zaverecny');
  ...
  return (
    <>
      <PageHero
        title={config.title}
        subtitle={`${config.subtitle} · ${totalQuestions} otázek · úspěch od ${minCorrectAnswers} správných (80 %) · platnost certifikátu 2 roky (v periodice BOZP)`}
      >
        <Link
          href="/lms"
