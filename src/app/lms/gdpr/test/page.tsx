...
export const metadata: Metadata = {
  title: "Demo test GDPR",
  description:
    "Vyzkoušejte si formát závěrečného testu GDPR – zkrácená ukázka se 10 otázkami.",
};

export default async function GdprDemoTestPage() {
  const config = getDemoQuizConfig(COURSE_SLUG);
  const session = await getLmsSession();

  if (!session) {
    redirect("/lms/login?redirect=%2Flms%2Fgdpr%2Ftest");
  }
  ...
  return (
    <>
      ...
      <Section>
        <p className="mb-6 rounded-xl border border-violet-200 bg-violet-50 px-4 py-3 text-sm text-violet-900">
